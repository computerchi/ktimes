...
class CalendarTool {
  ...
  IranianCalendar() {
    ...
    do {
      ...
      jump = jm - jp;
      if (this.irYear >= jm) {
        leapJ += Math.trunc(jump / 33) * 8 + Math.trunc((jump % 33) / 4);
        jp = jm;
      }
      j++;
    ...
    leapJ += Math.trunc(N / 33) * 8 + Math.trunc(((N % 33) + 3) / 4);
    if (jump % 33 == 4 && jump - N == 4) leapJ++;
    // And the same in the Gregorian date of Farvardin the first
    leapG =
      Math.trunc(this.gYear / 4) -
      Math.trunc(((Math.trunc(this.gYear / 100) + 1) * 3) / 4) -
      150;
  ...
  IranianDateToJDN() {
    ...
    return (
      ...
      (this.irMonth - 1) * 31 -
      Math.trunc(this.irMonth / 7) * (this.irMonth - 7) +
      this.irDay -
      1
    );
  }
  ...
  JDNToIranian() {
    ...
    if (k >= 0) {
      if (k <= 185) {
        this.irMonth = 1 + Math.trunc(k / 31);
        this.irDay = (k % 31) + 1;
        return;
      } else k -= 186;
    } else {
    ...
    this.irMonth = 7 + Math.trunc(k / 30);
    this.irDay = (k % 30) + 1;
  }
  ...
  julianDateToJDN(year, month, day) {
    return (
      Math.trunc(((year + Math.trunc((month - 8) / 6) + 100100) * 1461) / 4) +
      Math.trunc((153 * ((month + 9) % 12) + 2) / 5) +
      day -
      34840408
    );
  }
   ...
   */
  JDNToJulian() {
    const j = 4 * Math.trunc(this.JDN) + 139361631;
    const i = Math.trunc((j % 1461) / 4) * 5 + 308;
    this.juDay = Math.trunc((i % 153) / 5) + 1;
    this.juMonth = (Math.trunc(i / 153) % 12) + 1;
    this.juYear =
      Math.trunc(j / 1461) - 100100 + Math.trunc((8 - this.juMonth) / 6);
  }
  ...
  gregorianDateToJDN(year, month, day) {
    ...
    jdn =
      jdn -
      Math.trunc(
        (Math.trunc((year + 100100 + Math.trunc((month - 8) / 6)) / 100) * 3) /
          4
      ) +
      752;
    return jdn;
   ...
   */
  JDNToGregorian() {
    let j = 4 * Math.trunc(this.JDN) + 139361631;
    j =
      j +
      (((Math.trunc((4 * Math.trunc(this.JDN) + 183187720) / 146097) * 3) / 4) *
        4 -
        3908);
    const i = Math.trunc((j % 1461) / 4) * 5 + 308;
    this.gDay = Math.trunc((i % 153) / 5) + 1;
    this.gMonth = (Math.trunc(i / 153) % 12) + 1; // % 12) +1 are added by Ali Mahdi to ensure month between 1 & 12.
    this.gYear =
      Math.trunc(j / 1461) - 100100 + Math.trunc((8 - this.gMonth) / 6);
  }
}
